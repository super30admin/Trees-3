...
//Recursive solution with back tracking
//Reversing the changes made in the last step to make the result allign to the solution is called backtracking
//here pop operation is backtracking
// Time Complexity : O(n)
// Space Complexity : O(h) worst case O(n).
//1. Find root and traverse complete left or rigt root through recursion
//2. Push the left path elements leaf node is encountered and pop the last added element to maintain one path.
//3. Push the right path elements leaf node is encountered and pop the last added element to maintain one path.
class Solution {
public:
    vector<vector<int>> result;
    vector<vector<int>> pathSum(TreeNode* root, int sum) {
